...
@command()
def add(x: int = Argument(help="First number"), y: int = Argument(help="Second number")):
    """Add two integers."""
    print(f"{x} + {y} = {x + y}")


@command()
def multiply(x: float = Argument(help="First number"), y: float = Argument(help="Second number")):
    """Multiply two floats."""
    print(f"{x} * {y} = {x * y}")


@command()
def divide(x: float = Argument(help="Numerator"), y: float = Argument(help="Denominator")):
    """Divide two numbers."""
    if y == 0:
