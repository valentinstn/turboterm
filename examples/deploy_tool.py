...
def push(
    ...
    if dry_run:
        console.print(f"[u]DRY RUN[/u] — no changes will be made")
    ...
    if not dry_run:
        console.print(f"[b]Deployed[/b] successfully!")
    else:
...
def rollback(
    ...
    steps: int = Option(["--steps", "-s"], help="Number of versions to roll back", default=1),
):
...
def logs(
    ...
    console.print(f"[b]Fetching {lines} log lines[/b] from [red]{env}[/red] ({mode})...")
